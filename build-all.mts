...
//      minify: "esbuild",
  ...
  console.group(`Building ${outName} (react)`);
  await build(createConfig());
  console.groupEnd();
  builtNames.push(outName);
  console.log(`Built ${outName}`);
}

const outputs = fs
  .readdirSync("appbuild")
  .filter((f) => f.endsWith(".js") || f.endsWith(".css"))
  .map((f) => path.join("appbuild", f))
